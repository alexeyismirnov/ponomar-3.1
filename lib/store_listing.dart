...
/// Android application id used by RuStore deep links.
const String kAndroidPackageId = 'com.frjonah.ponomar_ru';
...
/// Candidate URIs for the RuStore listing, native app first then web.
List<Uri> ruStoreListingUris({String packageId = kAndroidPackageId}) {
  return [
    Uri.parse('rustore://apps.rustore.ru/app/$packageId'),
    Uri.parse('https://www.rustore.ru/catalog/app/$packageId'),
...
Future<bool> openRuStoreListing() => _launchFirstWorkingUri(ruStoreListingUris());
...
/// Opens the store for the current platform: App Store on Apple, RuStore on Android.
Future<bool> openStoreListing() {
  if (usesAppleAppStore()) {
    return openAppleAppStore();
  }
  return openRuStoreListing();
}

/// Prompt used after several launches.
/// iOS/macOS: native StoreKit review sheet.
/// Android: in-app dialog that opens RuStore (not Google Play).
